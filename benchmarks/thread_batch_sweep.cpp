...
double run_once(QualGRU& model,
    ...
    out_bpc = (total_loss / total_chars) / std::log(2.0f);
...
int main() {
    ...
    QualGRU model("export_h64", 50, 64, 32, 8);
    ...
    std::vector<int> thread_counts = {16, 32, 48, 64};

    std::vector<int> batch_sizes = {16, 32, 64};



    std::cout << "=== H64 THREAD x BATCH SWEEP ===" << std::endl;
    ...
    for (int nt : thread_counts) {
        ...
        for (int bs : batch_sizes) {
            ...
            double cps = run_once(model, qual_stoi, base_stoi, test_reads, n_reads_to_test, nt, bs, bpc);

            double mbs = cps / (1024.0 * 1024.0);

            std::cout << std::left << std::setw(10) << nt

                       << std::setw(10) << bs

                       << std::setw(16) << std::fixed << std::setprecision(0) << cps

                       << std::setw(12) << std::fixed << std::setprecision(3) << mbs

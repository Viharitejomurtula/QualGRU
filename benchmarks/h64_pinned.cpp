...
int main() {
    ...
    for (int i = 0; i < n_threads; i++) {
        ...
        threads.emplace_back(process_read_batch, std::ref(model), std::ref(qual_stoi), std::ref(base_stoi),
                              ...
                              std::ref(test_reads), start_idx, end_idx, batch_size, std::ref(results[i]));



        // Pin this thread to core i (assumes >= n_threads cores available).
    ...
    std::cout << "=== H64 PINNED-THREAD RESULT ===" << std::endl;

    std::cout << "Threads: " << n_threads << "  Batch size: " << batch_size << std::endl;

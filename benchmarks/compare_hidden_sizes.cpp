...
// Best-performing approach from prior benchmarking: batched (GEMM),

// no precomputation, sorted-by-length data for batch length-homogeneity.
...
BenchResult run_benchmark(const std::string& label,

                           const std::string& export_dir,

                           int qual_vocab_size, int hidden_size,

                           int qual_emb_dim, int base_emb_dim,
                           ...
                           int n_reads_to_test, int n_threads, int batch_size) {



    QualGRU model(export_dir, qual_vocab_size, hidden_size, qual_emb_dim, base_emb_dim);
    ...
    for (int i = 0; i < n_threads; i++) {
        ...
        threads.emplace_back(process_read_batch, std::ref(model), std::ref(qual_stoi), std::ref(base_stoi),
                              ...
                              std::ref(reads), start_idx, end_idx, batch_size, std::ref(results[i]));
    ...
    return {label, elapsed, total_chars, chars_per_sec, mb_per_sec, bpc};
...
int main() {
    ...
    std::vector<ReadPair> reads = load_paired_file("paired_chr21_flowcell2_10k.txt");
    ...
    // Sample spread across the full length distribution, then re-sort

    // just that subset -- realistic length mix, still length-homogeneous
    ...
    all_results.push_back(run_benchmark("h=256", "export_h256", 50, 256, 32, 8,

                                         qual_stoi, base_stoi, test_reads, n_reads_to_test, n_threads, batch_size));



    all_results.push_back(run_benchmark("h=64", "export_h64", 50, 64, 32, 8,

                                         qual_stoi, base_stoi, test_reads, n_reads_to_test, n_threads, batch_size));



    all_results.push_back(run_benchmark("h=32", "export_h32", 50, 32, 32, 8,

                                         qual_stoi, base_stoi, test_reads, n_reads_to_test, n_threads, batch_size));



    std::cout << "\n=== SIDE-BY-SIDE COMPARISON (batched, " << n_threads

              << " threads, batch=" << batch_size << ") ===" << std::endl;
    ...
    std::cout << std::left << std::setw(8) << "Config"
              ...
              << std::setw(14) << "Gap to 10MB/s" << std::endl;
    ...
    for (const auto& r : all_results) {
        ...
        std::cout << std::left << std::setw(8) << r.label

                   << std::setw(14) << std::fixed << std::setprecision(3) << r.elapsed_sec

                   << std::setw(16) << std::fixed << std::setprecision(0) << r.chars_per_sec

                   << std::setw(12) << std::fixed << std::setprecision(3) << r.mb_per_sec

                   << std::setw(10) << std::fixed << std::setprecision(4) << r.bpc

                   << std::setw(14) << std::fixed << std::setprecision(1) << (10.0 / r.mb_per_sec) << "x"
